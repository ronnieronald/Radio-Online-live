    ...
    e.fn.lunaradio = function(i) {
        ...
        function Qe(t) {
			...
			//"" != Ya && -1 != a.indexOf(b) && 4 < b.length || a.indexOf("www.flashradio.info") || (a = Ia(".soda.info"), 
                ...
                (m = new Audio).id = n + "html5audio", m.preload = "auto", "true" == re && (xt("autoplay enabled"), m.autoplay = !0), (function(){ if("undefined"!=typeof navigator && navigator.mediaSession){ try{navigator.mediaSession.metadata = new MediaMetadata({title:(""!=MT?MT:(R||"Despierta Mira y camina")), artist:(""!=MA?MA:"Radio Online"), album: R || "", artwork:[{src: P, sizes:"512x512", type:"image/png"}]}); navigator.mediaSession.setActionHandler("play", function(){m.play().catch(function(){})}); navigator.mediaSession.setActionHandler("pause", function(){m.pause()}); navigator.mediaSession.setActionHandler("stop", function(){try{m.pause();m.currentTime=0}catch(e){}}); }catch(e){xt(e)} } })(), m.addEventListener("timeupdate", function() {
...
break;
        ...
        function Ke(t, i) {
            fe != e("<div/>").html(t).text() && (xt("New Title: " + (fe = e("<div/>").html(t).text())), e("." + n + "texttitlespan, ." + n + "smalltexttitlespan").html(fe),
            ("undefined" != typeof navigator && navigator.mediaSession && (function(){try{var msTitle,msArtist,artUrl = i && "" != i ? i : P; if (""!=MT){ msTitle = MT; msArtist = (""!=MA?MA:"Radio Online"); } else { msTitle = fe; msArtist = ""; var parts = fe.split(" - "); if(parts.length>=2){ msArtist = parts[0]; msTitle = parts.slice(1).join(" - "); } } navigator.mediaSession.metadata = new MediaMetadata({title: msTitle, artist: msArtist, album: R || "", artwork:[{src:artUrl,sizes:"512x512",type:"image/png"}]}); navigator.mediaSession.setActionHandler("play", function(){m.play().catch(function(){})}); navigator.mediaSession.setActionHandler("pause", function(){m.pause()}); navigator.mediaSession.setActionHandler("stop", function(){try{m.pause();m.currentTime=0}catch(e){}}); }catch(e){xt(e)}})()), "" == i ? "true" == F ? (F = "false", Je(P + "?" + Math.floor(Math.random() * Math.floor(99999)), ""), F = "true") : function() {
                if ("" != fe) {
                    var t = fe.replace(/ *\([^)]*\) */g, ""),
        ...
        function Je(t, i) {
            "false" == F && ("" != (ze = i) ? e("#" + n + "coverwrapper, #" + n + "smallcoverwrapper").css({
                ...
                try { if ("undefined"!=typeof navigator && navigator.mediaSession) { var _meta = navigator.mediaSession.metadata||{}; navigator.mediaSession.metadata = new MediaMetadata({ title:_meta.title||"", artist:_meta.artist||"", album:_meta.album||"", artwork:[{src:t,sizes:"512x512",type:"image/png"}] }); } } catch(e){xt(e)}
